...
import { Box, Paper, Typography, Grid, Card, CardContent, Button, Drawer, List, ListItemButton, ListItemText, FormControl, Select, MenuItem, Divider, LinearProgress, Tooltip } from '@mui/material';
import { TrendingUp, Assessment, Speed, Logout, Dashboard as DashboardIcon, Add, FilterDrama, CheckCircle, Warning, Block, Cancel, Info } from '@mui/icons-material';
...
const MetricsPage = ({ onSwitchTab, onLogout, selectedSubject }) => {
  ...
  const StatCard = ({ value, label, color, bgColor, borderColor, icon, tooltip }) => (
    <Tooltip title={tooltip || ''} arrow placement="top">
      <Card elevation={0} sx={{ bgcolor: bgColor, border: `1px solid ${borderColor}`, borderRadius: 2, textAlign: 'center', height: '100%', cursor: tooltip ? 'help' : 'default' }}>
      ...
      <Box sx={{ flex: 1, bgcolor: '#F5F7FA' }}>
        ...
        <Box sx={{ p: 3 }}>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={2.4}><StatCard value={totalQuestions} label="Total Questions" color="#3B82F6" bgColor="#EFF6FF" borderColor="#DBEAFE" tooltip="Total number of questions asked by students" /></Grid>
            <Grid item xs={2.4}><StatCard value={metrics.usage?.today || 0} label="Today" color="#10B981" bgColor="#ECFDF5" borderColor="#D1FAE5" tooltip="Questions asked today" /></Grid>
            <Grid item xs={2.4}><StatCard value={metrics.usage?.this_week || 0} label="This Week" color="#F59E0B" bgColor="#FEF3C7" borderColor="#FDE68A" tooltip="Questions asked in the last 7 days" /></Grid>
            <Grid item xs={2.4}><StatCard value={metrics.usage?.avg_confidence ? (metrics.usage.avg_confidence * 100).toFixed(0) + '%' : '—'} label="Avg Confidence" color="#8B5CF6" bgColor="#F5F3FF" borderColor="#E9D5FF" tooltip="How confident the system is in its answers (higher = better)" /></Grid>
            <Grid item xs={2.4}><StatCard value={metrics.usage?.avg_latency_ms ? (metrics.usage.avg_latency_ms / 1000).toFixed(1) + 's' : '—'} label="Avg Response" color="#EC4899" bgColor="#FDF2F8" borderColor="#FCE7F3" tooltip="Average time to generate a complete answer" /></Grid>
          </Grid>

          {/* Row 2: Pipeline Funnel Visualization */}
          {totalQuestions > 0 && (
            <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: 3, border: '1px solid #E2E8F0' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                <Box sx={{ bgcolor: '#8B5CF6', p: 1, borderRadius: 1.5 }}><TrendingUp sx={{ color: 'white', fontSize: 22 }} /></Box>
                <Typography variant="h6" sx={{ fontWeight: 600 }}>Question Pipeline Funnel</Typography>
                <Typography variant="caption" sx={{ color: '#94A3B8', ml: 1 }}>How questions flow through each validation stage</Typography>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2 }}>
                {[
                  { label: 'Questions Asked', value: totalQuestions, color: '#3B82F6', bg: '#EFF6FF' },
                  { label: 'Passed Subject Check', value: metrics.layer1?.pass || 0, color: '#16A34A', bg: '#F0FDF4' },
                  { label: 'Proceeded to Answer', value: (metrics.layer2?.valid || 0) + (metrics.layer2?.warning || 0), color: '#F59E0B', bg: '#FFFBEB', sub: `${metrics.layer2?.valid || 0} valid + ${metrics.layer2?.warning || 0} warning` },
                  { label: 'Answers Generated', value: metrics.layer3?.success || 0, color: '#7C3AED', bg: '#F5F3FF' },
                ].map((step, i, arr) => (
                  <React.Fragment key={i}>
                    <Box sx={{ flex: 1, p: 2, bgcolor: step.bg, borderRadius: 2, textAlign: 'center', border: `1px solid ${step.color}22` }}>
                      <Typography variant="h3" sx={{ color: step.color, fontWeight: 700 }}>{step.value}</Typography>
                      <Typography variant="caption" sx={{ color: step.color, fontWeight: 600 }}>{step.label}</Typography>
                      {i > 0 && (
                        <Typography variant="caption" sx={{ display: 'block', color: '#94A3B8', mt: 0.5 }}>
                          {step.sub || `${totalQuestions > 0 ? ((step.value / totalQuestions) * 100).toFixed(0) : 0}% of total`}
                        </Typography>
                      )}
                    </Box>
                    {i < arr.length - 1 && (
                      <Typography sx={{ color: '#CBD5E1', fontSize: 24, fontWeight: 700 }}>→</Typography>
                    )}
                  </React.Fragment>
                ))}
              </Box>
          ...
          {/* Row 3: Pipeline Performance (All 3 Layers + Latency) */}
          <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: 3, border: '1px solid #E2E8F0' }}>
            ...
            <Grid container spacing={3} sx={{ mb: 3 }}>
              <Grid item xs={5}>
                <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #E2E8F0', height: '100%' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                    <Box sx={{ bgcolor: '#8B5CF6', p: 1, borderRadius: 1.5 }}><Assessment sx={{ color: 'white', fontSize: 22 }} /></Box>
                    <Typography variant="h6" sx={{ fontWeight: 600 }}>Confidence Distribution</Typography>
                  </Box>
                  <Typography variant="caption" sx={{ color: '#94A3B8', display: 'block', mb: 2 }}>How confident the system is in generated answers</Typography>
                  {[
                    { label: 'High Confidence (>70%)', value: metrics.confidence_distribution?.high || 0, color: '#16A34A', bg: '#DCFCE7', desc: 'Reliable answers' },
                    { label: 'Medium Confidence (40-70%)', value: metrics.confidence_distribution?.medium || 0, color: '#F59E0B', bg: '#FEF3C7', desc: 'Verify with textbook' },
                    { label: 'Low Confidence (<40%)', value: metrics.confidence_distribution?.low || 0, color: '#EF4444', bg: '#FEE2E2', desc: 'Check with professor' },
                  ].map((item, i) => {
                    const total = (metrics.confidence_distribution?.high || 0) + (metrics.confidence_distribution?.medium || 0) + (metrics.confidence_distribution?.low || 0);
                    const pct = total > 0 ? ((item.value / total) * 100).toFixed(0) : 0;
                    return (
                      <Box key={i} sx={{ mb: 2 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                          <Typography variant="body2" sx={{ fontWeight: 600, color: '#334155' }}>{item.label}</Typography>
                          <Typography variant="body2" sx={{ fontWeight: 700, color: item.color }}>{item.value} ({pct}%)</Typography>
                        </Box>
                        <Box sx={{ height: 12, bgcolor: '#F1F5F9', borderRadius: 6, overflow: 'hidden' }}>
                          <Box sx={{ height: 12, width: `${pct}%`, bgcolor: item.color, borderRadius: 6, transition: 'width 0.6s ease' }} />
                        </Box>
                        <Typography variant="caption" sx={{ color: '#94A3B8' }}>{item.desc}</Typography>
                      </Box>
                    );
                  })}
                </Paper>
            ...
            <Grid item xs={12}>
              <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #E2E8F0', height: '100%' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  ...
                  <Typography variant="caption" sx={{ color: '#94A3B8', ml: 1 }}>When students are most active (24-hour view)</Typography>
                </Box>
